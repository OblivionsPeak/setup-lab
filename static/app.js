...
async function upload(file) {
  ...
  try {
    ...
    if (!r.ok) return showError(data.error || 'Analysis failed.');
    render(data);
  } catch (err) {
    $('#progress').hidden = true;
    showError('Could not reach the analysis engine: ' + err.message);
  }
}
...
function render(data) {
  ...
  R.appendChild(el(`<div class="meta-bar">
    ...
    <div><span>Corners detected</span><b>${data.n_corners}</b></div>
  </div>`));
...
function drawBalanceChart(c, corners) {
  ...
  corners.forEach((k, i) => {
    ...
    const yy = y(Math.max(v, 0)), hh = Math.abs(y(v) - mid);
    ctx.fillRect(xx, v > 0 ? y(v) : mid, bw, Math.max(hh, 1));
    ctx.fillStyle = '#8b949e';
    ctx.fillText('T' + k.corner, xx + bw / 2 - 8, H - 8);
  });
}
// + = loose (oversteer), − = pushes (understeer): blend the two normalized proxies
...
async function loadHistory() {
  ...
  data.stints.forEach(s => {
    ...
    row.onclick = async () => {
      const rr = await fetch('/api/stint/' + s.id); const d = await rr.json();
